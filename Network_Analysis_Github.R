...
load("X:/Psychology/ResearchProjects/EWalton/EarlyCause/WP3/FS/Data/Housing_Data_with_SVs.RData")
df <- dataset
rm(dataset)
...
#Central Stability 
# For plotting, we have to use a caseMin of 0.1 to show the full range of dropped cases
casedrop_boot <- bootnet(network, nBoots = 1000, caseMin = 0.1, caseMax = 0.894, nCores = 8, type = "case", statistics = c("betweenness", "closeness", "expectedInfluence")) 
plot(casedrop_boot, statistics = c( "betweenness", "closeness", "expectedInfluence"))
# For getting the true values for tables, we have to specify caseMin to be higher so that it can more accurately distinguish between closeness and betweenness
casedrop_boot <- bootnet(network, nBoots = 1000, caseMin = 0.683, caseMax = 0.894, nCores = 8, type = "case", statistics = c("betweenness", "closeness", "expectedInfluence")) 
...
plot(bridges, include=c("Bridge Expected Influence (1-step)"), labels = F)
print(bridges)
